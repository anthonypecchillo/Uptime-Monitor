...
// Optional data: none
// NOTE: There is no reason to modify a token, other than its expiration time.
//       Rather than allowing a user to extend it an arbitrary amount of time,
//       it's better to just have them send an extend = true payload. Then we
//       can extend the expiration time out one hour from this moment.
...
handlers.checks = function(data, callback) {
  ...
  if (acceptableMethods.indexOf(requestedMethod) > -1) {
    handlers._checks[requestedMethod](data, callback);
  } else {
    callback(405);  // Method not allowed
  }
};
...
handlers._checks.post = function(data, callback) {
  ...
  var timeoutSeconds = typeof(data.payload.timeoutSeconds) == 'number' && data.payload.timeoutSeconds > 0 && data.payload.timeoutSeconds % 1 === 0 && data.payload.timeoutSeconds >= 1 && data.payload.timeoutSeconds <= 5 ? data.payload.timeoutSeconds : false;
  ...
  if (id) {
    ...
    _data.read('checks', id, function(err, checkData) {
      if (!err && checkData) {
        // Get the token from the headers
        var token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
        // Verify that the given token is valid and belongs to the user who created the check we're looking up
              ...
              if (!err) {
                ...
                _data.read('users', checkData.userPhone, function(err, userData) {
                  if (!err && userData) {
                    ...
                    callback(500, {'Error': 'Could not find the user who created the check, therefore could not remove the check from the list of checks on the user object.'});
                  }
                });
              } else {
                callback(500, {'Error': 'Could not delete the specified check data.'});
              }
            });
          } else {
            callback(403, {'Error': 'Missing required token in header, or token is invalid.'});
